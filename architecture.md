...
1. **Extension Activation**: VS Code loads the extension, which initializes the
   core logic.
2. **UI Rendering**: Extension creates WebViews and TreeViews, populating them with
   data from core modules.
3. **User Interactions**: User actions (e.g., button clicks) are handled by the
   extension layer and delegated to core logic.
4. **Data Updates**: Core logic updates state and notifies the extension layer for
   UI refreshes.
5. **Persistence**: Project save/load operations flow through the data persistence
   module.
